...
export const vehiclesRoutes = async (fastify) => {
    fastify.get('/', {
        schema: {
        ...
        const rows = await withTenantContext(pool, req.tenantId, async (c) => {
            ...
            vals.push(limit + 1);
            const { rows } = await c.query(`SELECT id,tenant_id,vin,plate,make,model,year,owner_name,owner_phone,created_at
    ...
    fastify.get('/:id', {
        ...
        const { id } = req.params;
        const rows = await withTenantContext(pool, req.tenantId, async (c) => {
            const { rows } = await c.query(`SELECT * FROM vehicles WHERE id=$1 AND deleted_at IS NULL`, [id]);
            return rows;
        });
        if (!rows.length)
    ...
    fastify.delete('/:id', {
        ...
        const { id } = req.params;
        const rows = await withTenantContext(pool, req.tenantId, async (c) => {
            const { rows } = await c.query(`UPDATE vehicles SET deleted_at = NOW()
