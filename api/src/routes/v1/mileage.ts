...
type MileageBodyType = Static<typeof MileageBody>;
...
type MileageQueryType = Static<typeof MileageQuerySchema>;

export const mileageRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get<{ Querystring: MileageQueryType }>('/', {
    ...
    let decoded: { id: string; recordedAt: string } | null = null;
    if (cursor) {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      const conds = [`vehicle_id = $1`]; const vals: unknown[] = [vehicleId]; let p = 2;
      if (decoded) {
      ...
      const { rows } = await c.query(
        `SELECT m.*, u.email AS recorded_by_email
         FROM mileage_history m LEFT JOIN users u ON u.id = m.recorded_by
         WHERE ${conds.join(' AND ')}
         ORDER BY m.recorded_at DESC, m.id DESC LIMIT $${p}`,
        vals,
      );
      return rows;
    });
  ...
  fastify.post<{ Body: MileageBodyType }>('/', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      ...
      const { rows: vRows } = await c.query(
        `SELECT id, current_mileage_km FROM vehicles WHERE id=$1 AND deleted_at IS NULL`,
        [vehicleId],
      );
      if (!vRows.length) throw Object.assign(new Error('Vehicle not found'), { statusCode: 404 });

      const { rows } = await c.query(
        `INSERT INTO mileage_history (tenant_id, vehicle_id, mileage_km, recorded_at, recorded_by, note)
         VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
        [req.tenantId, vehicleId, mileageKm,
         recordedAt ?? new Date().toISOString(), recordedBy, note ?? null],
      );

      // Keep vehicles.current_mileage_km as the highest recorded value
      await c.query(
        `UPDATE vehicles SET current_mileage_km = $2
         WHERE id=$1 AND (current_mileage_km IS NULL OR current_mileage_km < $2)`,
        [vehicleId, mileageKm],
      );
