...
type JobBodyType = Static<typeof JobBody>;
...
type JobPatchBodyType = Static<typeof JobPatchBody>;
...
type JobQueryType = Static<typeof JobQuerySchema>;

export const jobsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get<{ Querystring: JobQueryType }>('/', {
    ...
    let decoded: { id: string; performedAt: string } | null = null;
    if (cursor) {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      const conds: string[] = []; const vals: unknown[] = []; let p = 1;
      ...
      const { rows } = await c.query(
        `SELECT j.*, u.email AS created_by_email
         ...
         ${where}
         ORDER BY j.performed_at DESC, j.id DESC LIMIT $${p}`,
        vals,
      );
      return rows;
    });
  ...
  fastify.post<{ Body: JobBodyType }>('/', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      ...
      const { rows } = await c.query(
        `INSERT INTO jobs (tenant_id, vehicle_id, title, description, price_cents, performed_at, created_by)
         VALUES ($1,$2,$3,$4,$5,$6::date,$7) RETURNING *`,
        [req.tenantId, vehicleId, title, description ?? null,
  ...
  fastify.patch<{ Body: JobPatchBodyType }>('/:id', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      const { rows } = await c.query(
        ...
        [id, title ?? null, description ?? null, priceCents ?? null, performedAt ?? null],
      );
      return rows;
    });
