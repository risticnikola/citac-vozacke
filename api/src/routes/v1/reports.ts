...
type ReportBodyType = Static<typeof ReportBody>;

export const reportsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post<{ Body: ReportBodyType }>('/', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      ...
      const { rows: crRows } = await c.query(
        `SELECT id FROM card_reads WHERE id=$1`, [cardReadId],
      );
      if (!crRows.length)
        throw Object.assign(new Error('Card read not found'), { statusCode: 404 });

      const { rows } = await c.query(
        `INSERT INTO reports (tenant_id, card_read_id) VALUES ($1,$2) RETURNING *`,
        [req.tenantId, cardReadId],
      );
      return rows;
    });
