// api/src/routes/v1/service-reminders.ts
...
type ReminderQueryType = Static<typeof ReminderQuerySchema>;

export const serviceRemindersRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get<{ Querystring: ReminderQueryType }>('/', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      ...
      if (vehicleId)   { conds.push(`sr.vehicle_id = $${p++}`);   vals.push(vehicleId); }
      if (serviceType) { conds.push(`sr.service_type = $${p++}`); vals.push(serviceType); }

      if (status === 'open')      conds.push(`sr.completed_at IS NULL`);
      if (status === 'completed') conds.push(`sr.completed_at IS NOT NULL`);
  ...
  fastify.delete('/:id', {
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      const { rows } = await c.query(
        `DELETE FROM service_reminders WHERE id=$1 RETURNING id`, [id],
      );
      return rows;
    });
    if (!rows.length) return reply.code(404).send({ error: 'Not found' });
