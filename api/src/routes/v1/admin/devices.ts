// api/src/routes/v1/admin/devices.ts
import { FastifyPluginAsync } from 'fastify';
import { Type, Static } from '@sinclair/typebox';
import { randomUUID, randomBytes } from 'crypto';
import { pool } from '../../../db/client.js';
import { withTenantContext } from '../../../db/tenant-context.js';
import type { TenantJwtPayload } from '../../../types/index.js';
...
function generateActivationToken(): string {
  ...
  for (let g = 0; g < 4; g++) {
    let group = '';
    for (let i = 0; i < 4; i++) group += chars[bytes[g * 4 + i] % chars.length];
    groups.push(group);
  }
  return groups.join('-');
}
...
type GenerateTokenBodyType = Static<typeof GenerateTokenBody>;
...
type DeviceBodyType = Static<typeof DeviceBody>;

export const adminDevicesRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/', {
    preHandler: [fastify.authenticate, fastify.requireTenantContext],
  }, async (req, reply) => {
    const jwt = req.jwtPayload as TenantJwtPayload;
  ...
  fastify.post<{ Body: GenerateTokenBodyType }>('/generate-token', {
  ...
  fastify.post<{ Body: DeviceBodyType }>('/', {
    ...
    const { rows: limitRows } = await pool.query(
      `SELECT max_devices FROM tenants WHERE id=$1`, [req.tenantId],
    );
    ...
    const rows = await withTenantContext(pool, req.tenantId, async (c) => {
      ...
      const { rows } = await c.query(
        ...
        [req.tenantId, serial, name ?? null, platform ?? null,
