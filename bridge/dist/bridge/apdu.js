...
exports.ISO7816 = void 0;
exports.buildApdu = buildApdu;
exports.parseApduResponse = parseApduResponse;
function buildApdu(cmd) {
    const parts = [cmd.cla, cmd.ins, cmd.p1, cmd.p2];
    if (cmd.data && cmd.data.length > 0) {
        parts.push(cmd.data.length);
        for (const byte of cmd.data)
            parts.push(byte);
    }
    if (cmd.le !== undefined)
        parts.push(cmd.le);
...
function parseApduResponse(buf) {
    if (buf.length < 2)
...
exports.ISO7816 = {
    selectMasterFile: () => buildApdu({ cla: 0x00, ins: 0xA4, p1: 0x00, p2: 0x0C }),
    selectEfById: (fileId) => buildApdu({ cla: 0x00, ins: 0xA4, p1: 0x02, p2: 0x04, data: fileId }),
    readBinary: (offset, length) => buildApdu({ cla: 0x00, ins: 0xB0, p1: (offset >> 8) & 0x7F, p2: offset & 0xFF, le: length }),
    getChallenge: () => buildApdu({ cla: 0x00, ins: 0x84, p1: 0x00, p2: 0x00, le: 0x08 }),
};
